...
% Hard skill bars
\programming{{C $\textbullet$ HCL  $\textbullet$ YAML $\textbullet$ Go / 3}, {Bash $\textbullet$ Python $\textbullet$  Lua / 5}, {Terraform $\textbullet$ AWS $\textbullet$ Ansible / 5}, {Github Actions $\textbullet$ K8S $\textbullet$ Docker / 3.5}}
...
\begin{twenty} % Environment for a list with descriptions
	...
	{}
	{\begin{itemize}
		\item Architected and optimized 10+ AWS multi-account environments with AWS Organizations, Control Tower, and SCPs, ensuring zero downtime.
    \item Automated production server provisioning and configuration with Packer and Ansible, reducing setup time by 35\%.
    \item Reduced cloud costs by 30\% using AWS Savings Plans, Spot Instances, and Compute Optimizer.
    \item Designed high-availability architectures with Auto Scaling, ALB, and RDS Multi-AZ, achieving 99.99\% uptime.
    \item \parbox[t]{0.99\linewidth}{\raggedright Enhanced observability with CloudWatch, Prometheus, Grafana, cutting incident response time by 40\%.}
	  \end{itemize}}
	\\
	\twentyitem
	{Jun 2021 -}
